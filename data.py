...
#                               VALUES (?, ?, ?, ?, ?, ?,?,?,?,?)''',
#                            (product['category'], product['name'], exec(product['price1']), exec(product['price2']), exec(product['price3']), product['description']),product['picture'],product['url1'],product['url2'],product['url3'])
#         else:
#             # Product already exists, skip insertion
#             print(f"Skipping duplicate product: {product['name']}")
...
for category, products_list in products.items():
    for product in products_list:
        ...
        if existing_product is None:
            a=eval(product['price1'])
            b=eval(product['price2'])
            c=eval(product['price3'])
            # Product does not exist, insert it into the database
            cursor.execute(
                '''INSERT INTO products (category, name, price1, price2, price3, description, picture, url1, url2, url3)
                   VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)''',
                (product['category'], product['name'], a, b, c,
                 product['description'], product['picture'], product['url1'], product['url2'], product['url3'])
            )
        else:
            # Product already exists, skip insertion
            print(f"Skipping duplicate product: {product['name']}")
